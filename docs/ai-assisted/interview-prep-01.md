---
layout: default
title: Technical Interview Prep Master Summary
nav_order: 1
---
...
A framework for measuring how execution time (Time Complexity) and extra memory (Space Complexity) scale as input size ($N$) grows.
...
Space Complexity: Focuses strictly on newly created memory (e.g., mapping to a new array is $O(N)$, accumulating a total number variable is $O(1)$).
